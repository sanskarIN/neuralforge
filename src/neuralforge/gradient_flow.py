...
def group_gradient_stats(
    ...
    *,
    ...
    """Group parameters by the first label component, e.g. ``L0``/``L1``."""
...
def assess_gradient_health(
    ...
    *,
    ...
    """Classify one gradient snapshot using explicit configurable thresholds."""
    ...
    if stats.nonfinite_count:
        return GradientHealth(
            status="non_finite",
            message="one or more gradients are NaN or infinite",
            stats=stats,
        )
    if stats.l2_norm <= vanish:
        return GradientHealth(
            status="vanishing",
            message=f"gradient L2 norm {stats.l2_norm:.3e} is at/below {vanish:.3e}",
            stats=stats,
        )
    if stats.max_abs >= explode:
        return GradientHealth(
            status="exploding",
            message=f"maximum absolute gradient {stats.max_abs:.3e} is at/above {explode:.3e}",
            stats=stats,
        )
    return GradientHealth(
        status="healthy",
        message="gradient snapshot is finite and inside configured thresholds",
        stats=stats,
    )
