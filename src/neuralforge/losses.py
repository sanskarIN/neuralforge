...
def huber_loss(predictions: Iterable[Scalar], targets: Iterable[Scalar], *, delta: float = 1.0) -> Value:
...
def mean_categorical_cross_entropy_with_logits(batch_logits: Iterable[Sequence[Scalar]], target_indices: Iterable[int]) -> Value:
    pairs = list(zip(batch_logits, target_indices, strict=True))
    return _mean([categorical_cross_entropy_with_logits(logits, target) for logits, target in pairs])
...
def recommend_output_design(task: str, *, classes: int | None = None) -> OutputDesign:
    ...
    if normalized == "regression":
        return OutputDesign("regression", 1, "linear", "mse_or_huber", "continuous scalar", "use the scalar output directly")
    if normalized in {"binary", "binary_classification"}:
        return OutputDesign("binary_classification", 1, "linear_logits", "binary_cross_entropy_with_logits", "0/1 scalar", "apply sigmoid, then threshold probability")
    if normalized in {"multiclass", "multiclass_classification"}:
        ...
        return OutputDesign("multiclass_classification", classes, "linear_logits", "categorical_cross_entropy_with_logits", "integer class index", "apply softmax for probabilities; argmax for predicted class")
