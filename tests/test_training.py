...
class TrainingTests(unittest.TestCase):
    ...
    def test_invalid_inputs_are_rejected(self) -> None:
        ...
        with self.assertRaises(ValueError):
            ExperimentConfig(optimizer="unknown")  # type: ignore[arg-type]
        with self.assertRaises(TypeError):
            ExperimentConfig(seed=True)
        with self.assertRaises(ValueError):
            ExperimentConfig(clip_max_norm=0.0)
        with self.assertRaises(ValueError):
            run_regression_experiment([], [], [1])
        with self.assertRaises(ValueError):
            run_regression_experiment([[1.0], [1.0, 2.0]], [1.0, 2.0], [1])
        with self.assertRaises(ValueError):
            run_regression_experiment([[1.0]], [1.0], [2])
